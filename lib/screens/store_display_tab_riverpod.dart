...
/// تبويب عرض المتجر مع Riverpod
class StoreDisplayTabRiverpod extends ConsumerStatefulWidget {
  const StoreDisplayTabRiverpod({
    super.key,
  ...
  @override
  ConsumerState<StoreDisplayTabRiverpod> createState() =>
      _StoreDisplayTabRiverpodState();
}

class _StoreDisplayTabRiverpodState
    extends ConsumerState<StoreDisplayTabRiverpod> {
  ...
  Future<void> _initializeData() async {
    ...
    try {
      // تهيئة حالة تبويب عرض المتجر
      ref.read(storeDisplayStateProvider.notifier).initialize();
      ...
      if (mounted) {
        debugPrint(
            '🔄 تم جلب بيانات المخزون مباشرة في StoreDisplayTabRiverpod');
      }
  ...
  void _handleCardExpansion(String itemId, bool isExpanded) {
    ref.read(storeDisplayStateProvider.notifier).handleCardExpansion(
  ...
  Widget build(BuildContext context) {
    ...
    final hasItems = ref.watch(hasInventoryItemsProvider);
    final hasResults = ref.watch(hasFilteredResultsProvider);

    // التحقق من أن Provider مهيأ
    if (!storeDisplayState.isInitialized) {
      return const Center(
        child: Column(
    ...
    if (storeDisplayState.isDeleting) {
      return const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: <Widget>[
                ...
                _debounce = Timer(const Duration(milliseconds: 300), () {
                  if (mounted) {
                    try {
                      ...
                      ref
                          .read(storeDisplayStateProvider.notifier)
            ...
            children: <Widget>[
              ...
              ElevatedButton.icon(
                onPressed: () {
                  _searchController.clear();
                  // استخدام Riverpod لإعادة تعيين الفلتر
                  ref.read(storeDisplayStateProvider.notifier).resetFilter();
                },
  ...
  Widget _buildSortButton() => Container(
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
        ...
        child: PopupMenuButton<String>(
          onSelected: (String value) {
            ref.read(storeDisplayStateProvider.notifier).setSortBy(value);
          },
        ...
        child: PopupMenuButton<String>(
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: <Widget>[
                Icon(
                  ref.watch(storeDisplayStateProvider).sortAscending
                      ? Icons.arrow_upward
  ...
  Widget _buildStatsBar() {
    final stats = ref.watch(filteredInventoryStatsProvider);
          ...
          Expanded(
            child: _buildStatItem(
              ...
              CurrencyFormatter.formatCurrency(
                  (stats['totalValue'] as num).toDouble(), context),
              Icons.attach_money,
  ...
  Widget _buildInventoryCard(InventoryItem item, {Key? key}) {
    ...
    if (Platform.isWindows) {
      return WindowsInventoryCard(
        ...
        isExpanded: storeDisplayState.expandedItemId == item.id,
  ...
  Future<void> _testDataLoading() async {
    try {
      final result = await ref.read(testDataLoadingProvider.future);

      if (result['success'] == true) {
        if (mounted) {
          SnackbarUtils.showSuccess(
              context, 'تم تحميل ${result['totalCount']} عنصر');
        }
      } else {
        if (mounted) {
          SnackbarUtils.showError(
              context, 'خطأ في تحميل البيانات: ${result['error']}');
        }
      }
    } catch (e) {
  ...
  /// الحصول على تسمية الترتيب
  String _getSortLabel() {
    final sortBy = ref.watch(storeDisplayStateProvider).sortBy;
    switch (sortBy) {
      case 'name':
