...
class _DataCleanupScreenState extends State<DataCleanupScreen> {
  ...
  Future<void> _cleanupOldData() async {
    ...
    try {
      final CleanupResult result =
          await _cleanupService.cleanupOldData();
      setState(() {
        _lastResult = result.message;
        _currentStats = result.stats;
      });
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            if (_storageInfo != null) ...<Widget>[
              ...
              _buildInfoRow('عدد الملفات', _storageInfo!.fileCount.toString()),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            const Row(
              children: <Widget>[
                Icon(Icons.cleaning_services, color: AppConstants.primaryColor),
                SizedBox(width: 8),
                Text(
                  'خيارات التنظيف',
                  style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
