...
/// شاشة الإعدادات للوضع غير المتصل والإشعارات
...
class _SettingsTabState extends State<SettingsTab> {
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      _weeklyRemindersEnabled = prefs.getBool('weeklyRemindersEnabled') ?? true;
      _lowStockAlertsEnabled = prefs.getBool('lowStockAlertsEnabled') ?? true;
    });
  }

  // تمت إزالة تحميل حالة المزامنة

  // تمت إزالة المزامنة اليدوية

  // تمت إزالة مسح البيانات المحلية
              ...
              child: ListView(
                ...
                children: <Widget>[
                    ...
                    builder: (BuildContext context, AuthProvider auth, _) =>
                        auth.isAdmin
                            ? _buildModernSectionCard(
                                ...
                                isExpanded: _isUserMgmtExpanded,
                                onToggle: () => setState(() =>
                                    _isUserMgmtExpanded = !_isUserMgmtExpanded),
                              )
                  ...
                  _buildModernSectionCard(
                    ...
                    isExpanded: _isConnectionExpanded,
                    onToggle: () => setState(
                  ...
                  _buildModernSectionCard(
                    ...
                    isExpanded: _isRemindersExpanded,
                    onToggle: () => setState(
                        () => _isRemindersExpanded = !_isRemindersExpanded),
                  ),
                  const SizedBox(height: 20),
  ...
  /// بطاقة الإشعارات
  Widget _buildNotificationsCard() => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          _buildSwitchTile(
  ...
  /// بطاقة التذكيرات
  Widget _buildRemindersCard() => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          _buildSwitchTile(
  ...
  /// بطاقة حالة الاتصال
  Widget _buildConnectionStatusCard() => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
  ...
  /// بطاقة الإجراءات
  Widget _buildActionsCard() => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          _buildActionTile(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context).cleanupSuccessMessage),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                AppLocalizations.of(context).cleanupErrorMessage(e.toString())),
            backgroundColor: Colors.red,
          ),
        );
      }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                AppLocalizations.of(context).resetErrorMessage(e.toString())),
            backgroundColor: Colors.red,
          ),
        );
      }
